...
export async function GET() {
  try {
    ...
    const integration = await db.integration.findUnique({
      ...
      select: { id: true, portalId: true, syncStatus: true, lastSyncAt: true, lastSyncError: true, metadata: true, createdAt: true },
    });

    // Auto-reset stuck syncs after 10 minutes
    ...
    return NextResponse.json({ connected: !!integration, integration: integration || null });
  } catch {
