...
- ${config.subjectMode === "single"
    ...
    : `Subject lines must be short (3-6 words), cheeky and hyper-specific to that email's actual angle — not generic ("Quick question" is banned). Reference a real detail about the prospect/company where possible (e.g. a subject built around their specific product, a recent event, or the exact pain angle that email hits). Each subject must be distinct and must match its own email's content, not just the sequence's general theme.`}
