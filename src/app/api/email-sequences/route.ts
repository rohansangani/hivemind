...
- ${config.subjectMode === "single"
    ...
    : `Subject lines must be 4-8 words, cheeky and hyper-specific to that email's actual angle — not generic ("Quick question" is banned). They must create a curiosity gap or provocative angle, not just describe the topic. Style patterns to draw from (adapt these, don't reuse verbatim):
  ...
  A subject that just describes the email's topic in neutral words (e.g. "SoHo studio to doorstep") is too flat — it must make the reader want to know what happens next. Reference a real detail about the prospect/company where possible (their specific product, a recent event, or the exact pain angle that email hits). Each subject must be distinct and must match its own email's content, not just the sequence's general theme.`}
