...
1. GENERAL INSIGHTS: 1-3 SPECIFIC, concrete details useful for personalizing a cold email — recent news, a product launch, funding, a distinctive detail about what they actually do, a hiring signal, or an operational detail relevant to their industry. Be concrete, not generic industry filler ("the retail industry is evolving" is useless). RECENCY IS REQUIRED: only include details from ${currentYear} or the last few months — do NOT include something just because it's the most notable thing you found if it's actually old (e.g. a funding round or launch from ${currentYear - 2} or earlier). If you cannot find anything both specific AND recent, use the NONE marker below rather than reaching for stale information.
   ANY number or count you include (store count, employee count, funding amount, revenue, growth rate, years in business, or anything else quantified) must be something a page you actually found literally states — quote it the way the page states it, don't round it, adjust it, or estimate a "plausible" version of it. Confirmed live: a real brand with exactly one flagship store got reported here as "running 5 stores" — a fabricated count, not a misread one, since no page anywhere says 5. If you're not looking at a real number on a real page for that specific fact, describe the detail qualitatively instead ("runs a flagship store", "a small retail footprint") or drop that detail entirely rather than attach any figure to it.
