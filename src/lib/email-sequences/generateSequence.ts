...
- ${config.subjectMode === "single"
    ...
    : `Subject lines should be 5-8 words (aim for 6-7 as the sweet spot), hyper-specific to that email's actual angle — not generic ("Quick question" is banned) and never a bare unadorned label ("Shipping from Oklahoma City" alone, with no other flavor). NEVER include a number, percentage, or statistic in a subject line — no "92%", no "450+", no digit of any kind. Stats belong in the body only; the subject line's job is the hook, not the proof.
  ALMOST ALWAYS include the prospect's company name (or a short, natural form of it) directly in the subject line — this is true across nearly every style below, not just one of them (e.g. "Bliss Lau's most valuable real estate?", "Transforming 12th HOUSE's post-purchase journey", "Are Especia's returns a profit leak?"). Only skip the name if a subject genuinely reads better without it and still feels specific to that prospect, not generic.
  TONE: warm and conversational, like a genuinely curious person who did their homework — not a clever copywriter reaching for a punchy headline. Confident but soft, occasionally playful (a light pun on the company's own name or category), never aggressive or salesy, never a forced witty one-liner. First-person phrasing ("I enjoyed...", "I'll leave this here...") is welcome where it fits the style below.
  Six style patterns to draw from:
  ...
  - Cliffhanger/ellipsis: "I [tried/noticed/enjoyed] [specific product/experience] from [Company], but..."
...
- Team size / headcount: only state a specific number of employees (e.g. "your 18-person team", "a 13-person team") if that exact figure is explicitly present in the researched insights above — a web search finding one mention somewhere is not the same as a verified, current fact, and headcounts go stale fast. Never infer or estimate a headcount from other signals (store count, revenue, funding). If no explicit current headcount is given, describe team stage qualitatively instead ("a lean ops team", "a growing team") rather than committing to a specific number.
- ANY number or claim about the PROSPECT's own company (revenue, funding raised, order volume, growth rate, store count, ARR, valuation, employee count, or any other figure) must come verbatim from either the researched insights (web search/page content actually fetched for this prospect) above or an explicit field in the prospect information above — never invent, estimate, round, or infer one from company size/industry/vibe, no matter how plausible it sounds. Confirmed live: a sequence stated "₹25Cr+ in revenue" for a prospect with zero revenue data anywhere in context — pure fabrication, exactly what this rule exists to prevent. If you don't have a real number for the prospect, make the point qualitatively instead ("scaling fast", "a growing operation") or use a proof point from AVAILABLE STATS about a DIFFERENT (named reference) company instead — never attribute an invented number to the prospect itself.
