...
- ${config.subjectMode === "single"
    ...
    : `Subject lines must be 3-8 words, hyper-specific to that email's actual angle — not generic ("Quick question" is banned) and never a bare unadorned label ("Shipping from Oklahoma City" alone, with no other flavor). Vary the style across the sequence rather than repeating one formula every time — mix these freely (most real-world outreach leans on plain benefit-statements and named questions more than tricks, so don't force a hook onto every single line):
  ...
  Reference a real detail about the prospect/company where possible (their specific product, a recent event, or the exact pain angle that email hits). Each subject must be distinct and must match its own email's content, not just the sequence's general theme.`}
