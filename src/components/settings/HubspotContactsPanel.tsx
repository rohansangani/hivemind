...
function CompaniesBrowser() {
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      ...
      fetch(`/api/hubspot/companies?${params}`)
        .then(r => r.json())
    ...
    <div>
      ...
      <div className="max-h-[320px] overflow-y-auto space-y-1">
        ...
        {!loading && companies.length === 0 && (
          <p className="text-[11px] text-[var(--hm-text-tertiary)] py-2">No companies synced yet.</p>
        )}
        {companies.map(c => (
          <div key={c.id} className="flex items-center justify-between rounded-lg bg-[var(--hm-bg-secondary)] px-3 py-1.5">
            <div className="min-w-0">
              <p className="text-[12px] font-medium text-[var(--hm-text)] truncate">{c.name || "(unnamed)"}</p>
              <p className="text-[10px] text-[var(--hm-text-tertiary)] truncate">
            ...
            {c.numberOfEmployees ? (
              <span className="shrink-0 ml-2 text-[10px] font-medium rounded-full bg-[var(--hm-bg)] border border-[var(--hm-border)] px-2 py-0.5 text-[var(--hm-text-secondary)]">
                {c.numberOfEmployees.toLocaleString()} emp
              </span>
            ) : null}
          </div>
        ))}
    ...
    <div>
      ...
      <div className="max-h-[320px] overflow-y-auto space-y-1">
        ...
        {!loading && deals.length === 0 && (
          <p className="text-[11px] text-[var(--hm-text-tertiary)] py-2">No deals synced yet.</p>
        )}
        {deals.map(d => (
          <div key={d.id} className="flex items-center justify-between rounded-lg bg-[var(--hm-bg-secondary)] px-3 py-1.5">
            <div className="min-w-0">
              <p className="text-[12px] font-medium text-[var(--hm-text)] truncate">{d.dealName || "(unnamed deal)"}</p>
              <p className="text-[10px] text-[var(--hm-text-tertiary)] truncate">
                {d.pipeline ? `pipeline: ${d.pipeline}` : ""}{d.closeDate ? ` · close: ${new Date(d.closeDate).toLocaleDateString()}` : ""}
              </p>
            </div>
            <div className="shrink-0 ml-2 flex flex-col items-end gap-1">
              {d.amount ? (
                <span className="text-[10px] font-medium text-[var(--hm-text-secondary)]">${Math.round(d.amount).toLocaleString()}</span>
              ) : null}
              {d.dealStage && (
...
export default function HubspotContactsPanel() {
  ...
  return (
    <div className="px-5 py-4 space-y-5 border-t border-[var(--hm-border)]">
      ...
      <div>
        <div className="flex gap-1 mb-2 border-b border-[var(--hm-border)]">
          {(["contacts", "companies", "deals"] as const).map(t => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`px-3 py-1.5 text-[12px] font-medium capitalize border-b-2 -mb-px ${
                tab === t
                  ? "border-[var(--hm-primary)] text-[var(--hm-text)]"
                  : "border-transparent text-[var(--hm-text-tertiary)] hover:text-[var(--hm-text-secondary)]"
              }`}
            >
              {t}
            </button>
        ...
        {tab === "companies" && <CompaniesBrowser />}
        {tab === "deals" && <DealsBrowser />}
      </div>
    </div>
  );
}
